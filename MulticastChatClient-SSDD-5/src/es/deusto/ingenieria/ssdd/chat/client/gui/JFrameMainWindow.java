...
public class JFrameMainWindow extends JFrame implements Observer {
	...
	private void btnConnectClick() {
		if (!this.controller.isConnected()) {
			...
			if (this.txtFieldNick.getText().trim().equalsIgnoreCase("server")) {
				JOptionPane.showMessageDialog(this, "Nickname 'server' not allowed", "Connection initializarion error", JOptionPane.ERROR_MESSAGE);				
			...
			if (error == 1) {
				...
				if (!connectedUsers.isEmpty()) {
					...
					timerUsers = new Timer(2000, new ActionListener() {
						@Override
						public void actionPerformed(ActionEvent e) {
							updateConnectedUsersList();
						}
					});
					timerMessages = new Timer(500, new ActionListener() {
						@Override
						public void actionPerformed(ActionEvent e) {
